...
- 数据管理模块，支持从 MT5 同步并本地保存历史 K 线数据
- 交易复盘模块，提供带有播放控制的手动历史复盘与模拟交易环境
- 技术分析入口，可生成报告并使用系统默认浏览器打开
- 全面的多语言 (i18n) 支持，提供中英文界面的无缝切换
...
- `Data Management`：MT5 历史行情数据本地同步与管理
- `Trading Review`：支持行情回放与模拟开仓的手动复盘环境
