...
- Python Quant 模块，可创建绑定 MT5 账户的策略任务、回填本地行情并启动/停止 Python 策略执行
...
- `Python Quant`：绑定已配置 MT5 账户的 Python 策略任务与本地 SQLite 行情缓存
...
- 订单同步配置与运行记录
- 本地跟单配置与近期事件
- Python Quant 概览、任务生命周期控制与本地行情回填

## Python Quant 说明

- Python Quant 直接复用“本地跟单”中已经配置好的 MT5 账户。
