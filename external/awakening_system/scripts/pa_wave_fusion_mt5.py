# -*- coding: utf-8 -*-
"""
PA × Elliott 融合机构报告 · MT5 数据源入口
====================================================================
数据回退链：MT5 (Python 包) → MT4 (CSV 借用) → Yahoo Finance
报告输出   ：reports/fusion/fusion_mt5_<时间戳>.html  + fusion_mt5_latest.html

与现有所有入口完全互不干扰。
"""

import os
import sys

sys.path.insert(0, os.path.dirname(__file__))

from gold_analysis import fetch_data_mt5_first
from pa_wave_fusion import run_fusion_analysis


def main():
    return run_fusion_analysis(
        fetch_fn=fetch_data_mt5_first,
        report_prefix="fusion_mt5",
        report_subdir="fusion",
        system_label="MT5 系统",
    )


if __name__ == "__main__":
    main()
